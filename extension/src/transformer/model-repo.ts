...
const createBuiltinAssets = (): ActiveTransformerRuntimeAssets => ({
  ...
  modelLoadOptions: {
    // Built-in transformer currently uses remotely hosted ORT wasm runtime.
...
const ensureRuntimeWasmBaseUrl = async (repo: string): Promise<string> => {
  for (const runtimeDir of RUNTIME_DIR_CANDIDATES) {
    ...
    const wasmUrl = runtimeWasmUrlForBase(baseUrl);
    if (await urlLooksReachable(wasmUrl)) {
      return baseUrl;
    }
  }
  throw new Error(
...
export const resolveTransformerAssetsForSource = async (
  ...
  try {
    ...
    const runtimeWasmBaseUrlFromRecord = String(
      runRecord.runtime_wasm_base_url || "",
    ).trim();
    const runtimeWasmBaseUrl =
      runtimeWasmBaseUrlFromRecord ||
      (await ensureRuntimeWasmBaseUrl(normalizedSource.repo));
